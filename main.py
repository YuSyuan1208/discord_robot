...
async def on_ready():
    ...
    if team_fight_function_enable and team_fight_list_compare_enable:
      ...
      ''' now message object '''
      #[now] get data
      msg_tip.append(await run_channel.send(content=f'周,王,限制周 核對開始'))
      ...
      msg_tip.append(await run_channel.send(content=f'目前進度, 周:{week_dc}, 王:{king_dc}, 限制周:{limit_dc}'))
      week_sys = int(now['周'])
      king_sys = int(now['王'])
      limit_sys = int(now['limit_max_week'])
      msg_tip.append(await run_channel.send(content=f'系統進度, 周:{week_sys}, 王:{king_sys}, 限制周:{limit_sys}'))
      ...
      if(now_changed_content):
        ...
        now_save()
      msg_tip.append(await run_channel.send(content=f'周,王,限制周 核對完畢'))
